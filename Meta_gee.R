# R code to run GEE regression on binary response
...
# Run gee regression for association analysis 
rstList = foreach(i=1:n, .packages="geepack", .combine=rbind) %dopar% {
geeResult(dosage=as.numeric(c(geno1[i,-c(1:5)],geno2[i,-c(1:5)])),m=m1+m2,pheno=pheno, snpname=name[i])
}
print('p-val success')
